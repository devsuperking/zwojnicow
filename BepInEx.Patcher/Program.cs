...
namespace Zwojnicow.Patcher
{
	class Program
	{
		...
		{
			Console.WriteLine($"Zwojnicow Patcher v{Assembly.GetExecutingAssembly().GetName().Version}");
		...
		static bool PatchUnityExe(string managedDir, string unityOutputDLL, out string message)
		{
			...
			try
			{
				string injectedDLL = Path.GetFullPath($"{managedDir}\\Zwojnicow.Bootstrap.dll");
				File.WriteAllBytes(injectedDLL, EmbeddedResource.Get("Zwojnicow.Patcher.Zwojnicow.Bootstrap.dll"));
		...
		static bool VerifyAssembly(AssemblyDefinition unity, out string message)
		{
			...
			if (unity.MainModule.AssemblyReferences.Any(x => x.Name.Contains("Zwojnicow")))
			{
				canPatch = false;

				message += "This assembly has already been patched by Zwojnicow.\r\n";
			}
