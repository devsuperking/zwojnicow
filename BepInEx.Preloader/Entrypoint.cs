...
namespace Zwojnicow.Preloader
{
	...
	{
		// This is a list of important assemblies in Zwojnicow core folder that should be force-loaded
		...
		private static void LoadCriticalAssemblies()
		{
			foreach (string criticalAssembly in CriticalAssemblies)
			{
				try
				{
					Assembly.LoadFile(Path.Combine(Paths.ZwojnicowAssemblyDirectory, criticalAssembly));
				}
				catch (Exception)
				{
		...
		private static Assembly LocalResolve(object sender, ResolveEventArgs args)
		{
			...
			if (Utility.TryResolveDllAssembly(assemblyName, Paths.ZwojnicowAssemblyDirectory, out foundAssembly)
...
namespace Doorstop
{
	internal static class Entrypoint
	{
		...
		/// <summary>
		///     The main entrypoint of Zwojnicow, called from Doorstop.
		/// </summary>
		public static void Start()
		{
			...
			try
			{
				...
				// In some versions of Unity 4, Mono tries to resolve Zwojnicow.dll prematurely because of the call to Paths.SetExecutablePath
				...
				typeof(Entrypoint).Assembly.GetType($"Zwojnicow.Preloader.{nameof(PreloaderRunner)}")
