...
using Zwojnicow.Preloader.Patching;
using Zwojnicow.Preloader.RuntimeFixes;
using HarmonyLib;
using Mono.Cecil;
...
namespace Zwojnicow.Preloader
{
	/// <summary>
	///     The main entrypoint of Zwojnicow, and initializes all patchers and the chainloader.
	/// </summary>
	internal static class Preloader
	{
		...
		public static void Run()
		{
			try
			{
				...
				string consoleTile = $"Zwojnicow {version} - {Paths.ProcessName}";
				...
				AssemblyPatcher.AddPatcher(new PatcherPlugin
				{
					...
					TypeName = "Zwojnicow.Chainloader"
				});

				Logger.Log(LogLevel.Info,
					$"Loaded 1 patcher method from [Zwojnicow.Preloader {version}]");
		...
		/// <summary>
		///     Inserts Zwojnicow's own chainloader entrypoint into UnityEngine.
		/// </summary>
		...
		{
			if (assembly.MainModule.AssemblyReferences.Any(x => x.Name.Contains("Zwojnicow")))
				throw new Exception("Zwojnicow has been detected to be patched! Please unpatch before using a patchless variant!");
			...
			if (entryType == null)
				throw new Exception("The entrypoint type is invalid! Please check your config/Zwojnicow.cfg file");

			using (var injected = AssemblyDefinition.ReadAssembly(Paths.ZwojnicowAssemblyPath))
			{
		...
		/// <summary>
		///     Allocates a console window for use by Zwojnicow safely.
		/// </summary>
