...
namespace Zwojnicow.Logging
{
	...
	public class DiskLogListener : ILogListener
	{
		...
		public DiskLogListener(string localPath, LogLevel displayedLogLevel = LogLevel.Info, bool appendLog = false, bool includeUnityLog = false)
		{
			...
			while (!Utility.TryOpenFileStream(Path.Combine(Paths.ZwojnicowRootPath, localPath), appendLog ? FileMode.Append : FileMode.Create, out fileStream, share: FileShare.Read, access: FileAccess.Write))
			{
				if (counter == 5)
				{
